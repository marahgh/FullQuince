...
from  dorn import calc_bounds

def theoretical_dorn_bounds(ds: Synthetic)->dict:
    X = np.expand_dims(ds.x, axis=0)
    Y0 = np.expand_dims(ds.y0, axis=0)
    Y1 = np.expand_dims(ds.y1, axis=0)
    ...
    return {
        "axis_x": axis_x,
        "tau_sup": mu_T_sup - mu_C_inf,
        "tau_inf": mu_T_inf - mu_C_sup
    }
...
def plot_ignorance(trial_dir):
    ...
    # Our addition TODO
    tau_bound = theoretical_dorn_bounds(ds_test)
